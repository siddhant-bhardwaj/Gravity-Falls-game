...
function preload(){
  ...
  enter_img=loadImage("Images/enter.jpeg");
...
controller_img=loadImage("Images/controller.png");
}
...
door4.scale=0.5;
door4.visible = false;
 //e=createSprite(200,100,30,30);
enmy = createSprite(1150,150,10,10);
enter=createSprite(120,250,50,50);
enter.addImage(enter_img);
enmy.velocityX = -0.3;
...
move=createSprite(114,382,40,40);
move.addImage(controller_img);
move.scale=0.2;
...
player = createSprite(1065,350,20,20);
...
if(gameState==="start" && player.isTouching(enter)){
...
if(orangekey.isTouching(orangelock)){
gameState="attic";
...
if(gameState==="over"){
  text("YOU HAVE LOST! run towards return to play again",300,100);
}

if(gameState==="over" && mousePressed(re)){
  gameState="start";
  player.visible=true;
  reset();
}
...
function displayrules(){
  ...
  text("Run towards enter",571,450);
